module Comander
  module CLI
    class Devenv < Thor
      ...
      def up
        invoke 'comander:c_l_i:base:vagrant', ['devenv', 'up']
      end
      ...
      def ssh
        invoke 'comander:c_l_i:base:vagrant', ['devenv', 'ssh']
      end

      desc 'provision', 'Provisions the devenv environment'
      def provision
        exec_command('sudo apt-get update')
        invoke 'comander:c_l_i:base:vagrant', ['devenv', ['provision']]
      end

      desc 'reload', 'Reloads the devenv environment'
      def reload
        invoke 'comander:c_l_i:base:vagrant', ['devenv', ['reload']]
      end

      desc 'halt', 'Halts the devenv environment'
      def halt
        invoke 'comander:c_l_i:base:vagrant', ['devenv', ['halt']]
      end

      desc 'destroy', 'destroys the devenv environment'
      def destroy
        invoke 'comander:c_l_i:base:vagrant', ['devenv', ['destroy']]
      end
