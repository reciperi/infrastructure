...
module Locb
  ...
  class Deploy
    ...
    def run!(environment, branch, task = 'deploy')
    ...
    def load_capfile
      load File.expand_path(
        "#{File.dirname(__FILE__)}/deploy/application/#{options[:application]}/Capfile"
      )
    end
    ...
    def add_capistrano_extensions!
      ...
      Capistrano::DSL::Paths.send(:define_method, :stage_config_path) do
        Pathname.new fetch(
          :stage_config_path,
          File.expand_path("#{File.dirname(__FILE__)}/deploy/stages")
        )
      end
      ...
      Capistrano::DSL::Paths.send(:define_method, :deploy_config_path) do
        Pathname.new fetch(
          :deploy_config_path,
          File.expand_path(
            "#{File.dirname(__FILE__)}/deploy/application/#{app_name}.rb"
          )
        )
      end
      ...
      Capistrano::Application.send(:define_method, :capfile) do
        File.expand_path(
          "#{File.dirname(__FILE__)}/deploy/application/#{app_name}/Capfile"
        )
      end
    end
  end
